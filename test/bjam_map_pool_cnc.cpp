...
cntree::map_pool_cnc <uint32_t,uint32_t> VTC ;
typedef cntree::map_pool_cnc <uint32_t,uint32_t>::iterator myiterator ;
...
ostream & operator <<( ostream & salida , const cntree::map_pool_cnc<int,double> &M);
ostream & operator <<( ostream & salida , const cntree::multimap_pool_cnc<int,double> &M);
...
void SinRepeticiones ( void )
{   //---------------------------- begin ------------------------
    cntree::map_pool_cnc<int, double> M1 ;
    int i ;
    ...
    //
    //----------------------------------------------------------
    cntree::map_pool_cnc<int, double> M4 ( std::move( M1)) ;
    ...
    M1 = std::move ( M4);
    cntree::map_pool_cnc <int,double> M2 ( M1 );
    BOOST_CHECK ( M2.size() == 5);
    for ( i = 0 ; i < M2.size() ; ++i)
        BOOST_CHECK ( M1.pos(i) == M2.pos(i));

    cntree::map_pool_cnc<int,double>::iterator Alfa,Beta;

    for ( i = 0 ,Alfa = M1.begin() ; Alfa != M1.end() ; i++,Alfa ++)
        BOOST_CHECK ( (*Alfa) == M1.pos(i) );

    cntree::map_pool_cnc<int,double> M3 ( M1.begin() +1, M1.end() -1 ) ;
...
void ConRepeticiones ( void )
{   //---------------------------- begin ------------------------
    cntree::multimap_pool_cnc<int, double> M1 ;
    int i ;
    ...
    cntree::multimap_pool_cnc<int, double> M4( std::move(M1)) ;
    ...
    cntree::map_pool_cnc <int,double> M5 ;
    M5.insert( A) ;
    M5.insert( B) ;
    M5.insert( C) ;
    M5.insert( D) ;
    ...
    M5.insert ( C);
    M5.insert( D) ;
    cntree::multimap_pool_cnc <int,double> M2 ( M5 );
    M2 = M1 ;
    ...
    cntree::multimap_pool_cnc<int,double>::iterator Alfa,Beta;

    for ( i =0, Alfa = M1.begin() ; Alfa != M1.end() ; i++,Alfa ++)
        BOOST_CHECK (*Alfa == M1.pos(i) );

    cntree::multimap_pool_cnc<int,double> M3 ( M1.begin() +1, M1.end() -1 ) ;
...
{   //---------------------- Variables----------------------------
    ...
    cntree::map_pool_cnc<int,double>::iterator Gamma ;
    for ( i = 0 ; i < M1.size() ; ++i)
    ...
    std::pair <cntree::map_pool_cnc<int, double>::iterator,
               cntree::map_pool_cnc<int, double>::iterator > PI ;
    //--------------- loop for existing elements ---------------------------
    for ( i = 0 ; i < 1000 ; i ++ )
...
{   //---------------------- Variables----------------------------
    ...
    // -------- Loop for find existing elements -------------------
    cntree::multimap_pool_cnc<int, double>::iterator Gamma ;
    for ( i = 0 ; i < 1000 ; ++i)
    {   Gamma = M1.find( 1000+ i * 2);
    ...
    std::pair < cntree::multimap_pool_cnc<int, double>::iterator,
                cntree::multimap_pool_cnc<int,double>::iterator > PI ;
    //--------------- loop for existing elements ---------------------------
    for ( i = 0 ; i < 1000 ; i ++ )
    {   PI = M1.equal_range(1000 + i *2 );
