//----------------------------------------------------------------------------
/// @file test_map.cpp
/// @brief Test program of the clases countertree::map_pool and countertree::multimap_pool
///
...
ostream & operator <<( ostream & salida , const cntree::map_pool<int,double> &M);
ostream & operator <<( ostream & salida , const cntree::multimap_pool<int,double> &M);
...
{   //---------------------------- begin ------------------------
    ...
    cntree::map_pool_cnc<int,double,std::less<int>, std::allocator<int> > MX (M1);
    BOOST_CHECK ( MX.size() == 5);
    MX.clear() ;
    MX = M1 ;
    BOOST_CHECK ( MX.size() == 5);
    cntree::map_pool<int,double,std::less<int>, std::allocator<int> > MY (std::move( MX));;
    BOOST_CHECK ( MY.size() == 5 and MX.size() == 5);
    MX = std::move ( MY);
    BOOST_CHECK ( MY.size() == 5 and MX.size() == 5);
    //----------------------------------------------------------
    //
    //----------------------------------------------------------
    cntree::map_pool<int, double> M4 ( std::move( M1)) ;
    ...
    M1 = std::move ( M4);
    cntree::map_pool <int,double> M2 ( M1 );
    BOOST_CHECK ( M2.size() == 5);
    for ( i = 0 ; i < M2.size() ; ++i)
        BOOST_CHECK ( M1.pos(i) == M2.pos(i));

    cntree::map_pool<int,double>::iterator Alfa,Beta;

    for ( i = 0 ,Alfa = M1.begin() ; Alfa != M1.end() ; i++,Alfa ++)
        BOOST_CHECK ( (*Alfa) == M1.pos(i) );

    cntree::map_pool<int,double> M3 ( M1.begin() +1, M1.end() -1 ) ;
...
void ConRepeticiones ( void )
{   //---------------------------- begin ------------------------
    cntree::multimap_pool<int, double> M1 ;
    int i ;
    ...
    cntree::multimap_pool<int, double> M4( std::move(M1)) ;
    ...
    M5.insert ( C);
    M5.insert( D) ;
    cntree::multimap_pool <int,double> M2 ( M5 );
    M2 = M1 ;
    //countertree::multimap_pool <int,double> M2 ( M1 );
    ...
    cntree::multimap_pool_cnc <int,double,std::less<int>,std::allocator<int> > MMX ( M2);
    BOOST_CHECK ( MMX.size() == 8 );
    MMX.clear() ;
    MMX= M2 ;
    BOOST_CHECK ( MMX.size() == 8 );
    cntree::map_pool_cnc <int,double,std::less<int>,std::allocator<int> > MX ( M1.begin(), M1.end());
    BOOST_CHECK (MX.size() == 4 )    ;
    cntree::multimap_pool <int,double> MN ( MX);
    BOOST_CHECK ( MN.size() == 4 );

    //------------------- rvalues -------------------------------------
    cntree::multimap_pool_cnc<int,double, std::less<int> ,std::allocator<void> > MMZ ( std::move(M2));
    BOOST_CHECK ( MMZ.size() == 8  and M2.size() == 8);
    M2 = std::move ( MMZ);
    BOOST_CHECK ( MMZ.size() == 8  and M2.size() == 8);

    cntree::multimap_pool<int,double>::iterator Alfa,Beta;

    for ( i =0, Alfa = M1.begin() ; Alfa != M1.end() ; i++,Alfa ++)
        BOOST_CHECK (*Alfa == M1.pos(i) );

    cntree::multimap_pool<int,double> M3 ( M1.begin() +1, M1.end() -1 ) ;
...
{   //---------------------- Variables----------------------------
    ...
    cntree::map_pool<int,double>::iterator Gamma ;
    for ( i = 0 ; i < M1.size() ; ++i)
    ...
    std::pair <cntree::map_pool<int, double>::iterator, cntree::map_pool<int, double>::iterator > PI ;
    //--------------- loop for existing elements ---------------------------
    for ( i = 0 ; i < 1000 ; i ++ )
...
{   //---------------------- Variables----------------------------
    ...
    // -------- Loop for find existing elements -------------------
    cntree::multimap_pool<int, double>::iterator Gamma ;
    for ( i = 0 ; i < 1000 ; ++i)
    {   Gamma = M1.find( 1000+ i * 2);
    ...
    std::pair < cntree::multimap_pool<int, double>::iterator, cntree::multimap_pool<int,double>::iterator > PI ;
    //--------------- loop for existing elements ---------------------------
    for ( i = 0 ; i < 1000 ; i ++ )
    {   PI = M1.equal_range(1000 + i *2 );
...
{   //---------------------------- begin ------------------------
    ...
    typedef cntree::map_pool_cnc<int, double>::iterator iterator ;

    cntree::map_pool_cnc<int, double> M1,M2;
...
{   //---------------------------- begin ------------------------
    typedef std::pair<const int, double> value_t ;
    typedef  cntree::multimap_pool<int, double>::iterator iterator ;

    cntree::multimap_pool<int, double> M1,M2;
