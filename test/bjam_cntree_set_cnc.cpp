...
cntree_set <uint32_t, true> VTC ;
typedef cntree_set <uint32_t,true>::iterator s_iterator ;
...
void prueba1()
{   //------------------ begin ---------------
    cntree_set<int,true> s;
    int i ;
    cntree_set<int,true>::iterator Alfa, Beta;
    s.insert(1);
    s.insert(3);
    s.insert(5);
    s.insert(7);
    ...
    // Pruebas con rvalues
    //---------------------------------------------------------
    cntree_set<int,true> s2( std::move (s));

    for ( i = 1 , Alfa = s2.begin();Alfa != s2.end(); ++Alfa, i+=2)
        BOOST_CHECK ( *Alfa == i );
    s = std::move(s2);
    s2= s ;
    cntree_set<int,true>::reverse_iterator Delta ;
    ...
    cntree_multiset<int,true> ms(s2);
    ms.clear();
    ...
    // Pruebas con rvalues
    //-----------------------------------------------------
    cntree_multiset<int,true> ms2(std::move (ms));
...
{   //------------------- begin -------------------------
    ...
    cntree_set<int,true> A ;
    for ( int i = 0 ; i< NELEM ; ++i)
        A.insert( V[i] );

    cntree_set<int,true> B(A) ;
    BOOST_CHECK ( A.size() == B.size());
    cntree_set<int,true> C ( B.begin() + 20 , B.end() -20);
    B = C ;
    BOOST_CHECK ( C.size() == B.size() and A.size() != B.size());

    cntree_set<int,true>::iterator Alfa = A.begin() ;
    ...
    cntree_set<int,true>::iterator Beta ;
    ...
    std::pair<cntree_set<int,true>::iterator,cntree_set<int,true>::iterator> Gama ;
...
{   //-------------------- begin -------------------------
    ...
    cntree_multiset<int,true> B(A) ;
    BOOST_CHECK  (A.size() == B.size() );
    cntree_multiset<int,true> C ( B.begin() + 20 , B.end() -20);
    B = C ;
    BOOST_CHECK  (C.size() == B.size() and A.size() != B.size() );

    cntree_multiset<int,true>::iterator Alfa = A.begin() ;
    for (int  i = 0 ; Alfa != A.end() ; ++Alfa, ++i )
    {   BOOST_CHECK ( (*Alfa) == A.pos(i) );;
    };
    B.clear();
    ...
    cntree_multiset<int,true>::iterator Beta ;
    Alfa = A.lower_bound( *(A.begin() +10)-1 );
    Beta = A.upper_bound ( A.pos ( A.size() -10) +1);
    signed_type Dif = Beta - Alfa ;
    for ( int32_t i =0 ; i < Dif ; Alfa++,i++);
    ...
    std::pair<cntree_multiset<int,true>::iterator,
              cntree_multiset<int,true>::iterator> Gama ;
    Gama = A.equal_range(*(A.begin() +10) -1);
    Dif = Beta - Alfa ;
    for ( int32_t i =0 ; i < Dif ; Alfa++,i++);
    BOOST_CHECK ( Alfa == Beta );
...
{   //------------------ Variables----------------
    ...
    cntree_set<int,true>::iterator Gamma ;
    for ( i = 0 ; i < M1.size() ; ++i)
    ...
    std::pair < cntree_set<int,true>::iterator, cntree_set<int,true>::iterator > PI ;
...
{   //---------------------- Variables----------------------------
    ...
    cntree_multiset<int,true>::iterator Gamma ;
    for ( i = 0 ; i < 1000 ; ++i)
    {   Gamma = M1.find( 1000+ i * 2);
    ...
    std::pair < cntree_multiset<int,true>::iterator, cntree_multiset<int,true>::iterator > PI ;
