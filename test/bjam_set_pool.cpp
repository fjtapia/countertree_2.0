//----------------------------------------------------------------------------
/// @file test_set.cpp
/// @brief Test program of the classes countertree::set_pool and countertree::multiset
///
...
void prueba1()
{   //------------------ begin ---------------
    cntree::set_pool<int> s;
    int i ;
    cntree::set_pool<int>::iterator Alfa, Beta;
    s.insert(1);
    s.insert(3);
    s.insert(5);
    s.insert(7);
    ...
    // Pruebas con rvalues
    //---------------------------------------------------------
    cntree::set_pool<int> s2( std::move (s));

    for ( i = 1 , Alfa = s2.begin();Alfa != s2.end(); ++Alfa, i+=2)
        BOOST_CHECK ( *Alfa == i );
    s = std::move(s2);
    s2= s ;
    cntree::set_pool<int>::reverse_iterator Delta ;
    ...
    // Pruebas con rvalues
    //-----------------------------------------------------
    cntree::multiset<int> ms2(std::move (ms));
...
void prueba2()
{   //------------------ begin ---------------
    cntree::set_pool_cnc<int, std::less<int>, std::allocator <int> > SAux;
    for ( int i =0 ; i < 100 ; ++i)
        SAux.insert ( i );
    ...
    cntree::set_pool<int> S1 ( SAux)    ;
    //cntree::set_pool<int,false, std::less<int>> S1 ( SAux)    ;
    ...
    //---------------------------------------------------------------
    cntree::set_pool_cnc<int, std::less<int>,std::allocator<void> > SC ( std::move(S1));
    BOOST_CHECK ( SC.size() == 100 and S1.size() == 100);
    for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( SC.pos(i) == i) ;

    S1 = std::move(SC) ;
    BOOST_CHECK ( S1.size() == 100 and SC.size() == 100);
    for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( S1.pos(i) == i) ;

    cntree::set_pool<int> S3 ( S1.begin() , S1.end());
...
void prueba3()
{   //------------------ begin ---------------
    cntree::multiset_cnc<int,std::less<int>, std::allocator <int> > SAux;
    for ( int i =0 ; i < 100 ; ++i)
        SAux.insert ( i );
    BOOST_CHECK ( SAux.size() == 100);
    for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( SAux.pos(i) == i) ;
    cntree::set_pool<int,std::less<int>, std::allocator <int> > SAux2( SAux.begin(), SAux.end());
    cntree::multiset_cnc<int,std::less<int>, std::allocator <int> > SAux3( SAux2);
    ...
    for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( S1.pos(i) == i) ;
    cntree::set_pool_cnc<int >  SX (SAux2);
    BOOST_CHECK ( SX.size() == 100);
    cntree::multiset<int> MS1 ( std::move ( SX));
    ...
    BOOST_CHECK ( MS1.size() == 100);
     for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( MS1.pos(i) == i) ;
    //---------------------------------------------------------------
    cntree::multiset_cnc<int, std::less<int>,std::allocator<void> > SC ( std::move(S1));
    ...
    cntree::multiset<int> S3 ( S1.begin() , S1.end());
    BOOST_CHECK ( S3.size() == 100 );
    for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( S3.pos(i) == i) ;
...
{   //------------------- begin -------------------------
    ...
    cntree::set_pool<int> A ;
    for ( int i = 0 ; i< NELEM ; ++i)
        A.insert( V[i] );

    cntree::set_pool<int> B(A) ;
    BOOST_CHECK ( A.size() == B.size());
    cntree::set_pool<int> C ( B.begin() + 20 , B.end() -20);
    B = C ;
    BOOST_CHECK ( C.size() == B.size() and A.size() != B.size());

    cntree::set_pool<int>::iterator Alfa = A.begin() ;
    ...
    cntree::set_pool<int>::iterator Beta ;
    ...
    std::pair<cntree::set_pool<int>::iterator,cntree::set_pool<int>::iterator> Gama ;
...
{   //------------------ Variables----------------
    ...
    cntree::set_pool<int>::iterator Gamma ;
    for ( i = 0 ; i < M1.size() ; ++i)
    ...
    std::pair < cntree::set_pool<int>::iterator, cntree::set_pool<int>::iterator > PI ;
...
{   //---------------------- Variables----------------------------
    ...
    cntree::multiset<int>::iterator Gamma ;
    for ( i = 0 ; i < 1000 ; ++i)
    {   Gamma = M1.find( 1000+ i * 2);
    ...
    std::pair < cntree::multiset<int>::iterator, cntree::multiset<int>::iterator > PI ;
