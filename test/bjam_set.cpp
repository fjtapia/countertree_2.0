...
{   //------------------ begin ---------------
    ...
    // Pruebas con rvalues
    //---------------------------------------------------------
    cntree::set<int> s2( std::move (s));

    for ( i = 1 , Alfa = s2.begin();Alfa != s2.end(); ++Alfa, i+=2)
        BOOST_CHECK ( *Alfa == i );
    s = std::move(s2);
    s2= s ;
    cntree::set<int>::reverse_iterator Delta ;
...
void prueba2()
{   //------------------ begin ---------------
    cntree::set_cnc<int, std::less<int>, std::allocator <int> > SAux;
    for ( int i =0 ; i < 100 ; ++i)
        SAux.insert ( i );
    ...
    cntree::set<int> S1 ( SAux)    ;
    //cntree::set<int,false, std::less<int>> S1 ( SAux)    ;
    ...
    //---------------------------------------------------------------
    cntree::set_cnc<int, std::less<int>,std::allocator<void> > SC ( std::move(S1));
    ...
    cntree::set<int> S3 ( S1.begin() , S1.end());
...
{   //------------------ begin ---------------
    ...
    for ( int i = 0 ; i < 100 ; ++i)
        BOOST_CHECK ( SAux.pos(i) == i) ;
    cntree::set<int,std::less<int>, std::allocator <int> > SAux2( SAux.begin(), SAux.end());
...
{   //------------------ Variables----------------
    ...
    std::pair < cntree::set<int>::iterator, cntree::set<int>::iterator > PI ;
