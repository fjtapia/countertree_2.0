...
cntree_map <uint32_t,uint32_t, true> VTC ;
typedef cntree_map <uint32_t,uint32_t,true>::iterator myiterator ;
...
ostream & operator <<( ostream & salida , const cntree_map<int,double,true> &M);
ostream & operator <<( ostream & salida , const cntree_multimap<int,double,true> &M);
...
{   //---------------------------- begin ------------------------
    ...
    //
    //----------------------------------------------------------
    cntree_map<int, double,true> M4 ( std::move( M1)) ;
    ...
    M1 = std::move ( M4);
    cntree_map <int,double,true> M2 ( M1 );
    BOOST_CHECK ( M2.size() == 5);
    for ( i = 0 ; i < M2.size() ; ++i)
        BOOST_CHECK ( M1.pos(i) == M2.pos(i));

    cntree_map<int,double,true>::iterator Alfa,Beta;

    for ( i = 0 ,Alfa = M1.begin() ; Alfa != M1.end() ; i++,Alfa ++)
        BOOST_CHECK ( (*Alfa) == M1.pos(i) );

    cntree_map<int,double,true> M3 ( M1.begin() +1, M1.end() -1 ) ;
...
void ConRepeticiones ( void )
{   //---------------------------- begin ------------------------
    cntree_multimap<int, double,true> M1 ;
    int i ;
    ...
    cntree_multimap<int, double,true> M4( std::move(M1)) ;
    ...
    M5.insert ( C);
    M5.insert( D) ;
    cntree_multimap <int,double,true> M2 ( M5 );
    M2 = M1 ;
    ...
    cntree_multimap<int,double,true>::iterator Alfa,Beta;

    for ( i =0, Alfa = M1.begin() ; Alfa != M1.end() ; i++,Alfa ++)
        BOOST_CHECK (*Alfa == M1.pos(i) );

    cntree_multimap<int,double,true> M3 ( M1.begin() +1, M1.end() -1 ) ;
...
{   //---------------------- Variables----------------------------
    ...
    cntree_map<int,double,true>::iterator Gamma ;
    for ( i = 0 ; i < M1.size() ; ++i)
    ...
    std::pair <cntree_map<int, double,true>::iterator,
               cntree_map<int, double,true>::iterator > PI ;
    //--------------- loop for existing elements ---------------------------
    for ( i = 0 ; i < 1000 ; i ++ )
...
{   //---------------------- Variables----------------------------
    ...
    // -------- Loop for find existing elements -------------------
    cntree_multimap<int, double,true>::iterator Gamma ;
    for ( i = 0 ; i < 1000 ; ++i)
    {   Gamma = M1.find( 1000+ i * 2);
    ...
    std::pair < cntree_multimap<int, double,true>::iterator,
                cntree_multimap<int,double,true>::iterator > PI ;
    //--------------- loop for existing elements ---------------------------
    for ( i = 0 ; i < 1000 ; i ++ )
    {   PI = M1.equal_range(1000 + i *2 );
