...
int InsercionBinaria ( const cntree::vector_tree<int> &A , int Valor) ;
int BusquedaBinaria ( const cntree::vector_tree<int> &A , int Valor);


void prueba1 (void)
{   //------------------------ Inicio -----------------------------
    cntree::vector_tree<int32_t > A ;
    ...
    cntree::vector_tree<int32_t> B ( A.begin(), A.end());
    BOOST_CHECK (B.check() );

    for ( int i =0  ; i < 10 ;++i)  BOOST_CHECK(B[i] == i );

    cntree::vector_tree <int32_t> C ( (signed_type) 10, (int32_t)7 );
    BOOST_CHECK (C.check() );
    for ( int i =0  ; i < 10 ;++i)
        BOOST_CHECK(C[i] == 7 );

    //std::cout<<A<<std::endl ;
    cntree::vector_tree<int32_t> D( A);
    BOOST_CHECK (D.check() );
    //std::cout<<D<<std::endl ;
    for ( int i =0  ; i < 10 ;++i)
        BOOST_CHECK(D[i] == i );

    cntree::vector_tree_cnc <int32_t,std::allocator<uint32_t> > K (D);
    BOOST_CHECK ( K.size() == 10);
    K.clear() ;
    K = D ;
    BOOST_CHECK ( K.size() == 10);

    cntree::vector_tree_cnc <int32_t,std::allocator<void> > Q ( std::move ( D));
    ...
    //-------------------------------------------------------------------
    cntree::vector_tree<int32_t> H( std::move(A));
    BOOST_CHECK (H.check() );
    BOOST_CHECK (A.check() );
    ...
    cntree::vector_tree<int32_t>::iterator PAux  ;
    for ( int32_t i = 0 ; i < 10 ; i ++ )
    ...
    cntree::vector_tree<int32_t>::reverse_iterator Alfa , Beta ;
...
void prueba4 ( void)
{   //---------------------- Variables----------------------------
    cntree::vector_tree<int> M1, M2, M3 ;
       int i ;
...
void prueba7 ( void)
{   //--------------------------- begin --------------------------------------
    cntree::vector_tree<int> V1, V2,V3,V4 ;
