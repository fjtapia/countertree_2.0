...
using cntree::myallocator ;

int InsercionBinaria ( const cntree::vector_tree_pool<int> &A , int Valor) ;
int BusquedaBinaria ( const cntree::vector_tree_pool<int> &A , int Valor);


void prueba1 (void)
{   //------------------------ Inicio -----------------------------
    cntree::vector_tree_pool<int32_t > A ;
    ...
    cntree::vector_tree_pool<int32_t> B ( A.begin(), A.end());
    BOOST_CHECK (B.check() );

    for ( int i =0  ; i < 10 ;++i)  BOOST_CHECK(B[i] == i );

    cntree::vector_tree_pool <int32_t> C ( (signed_type) 10, (int32_t)7 );
    BOOST_CHECK (C.check() );
    for ( int i =0  ; i < 10 ;++i)
        BOOST_CHECK(C[i] == 7 );

    //std::cout<<A<<std::endl ;
    cntree::vector_tree_pool<int32_t> D( A);
    BOOST_CHECK (D.check() );
    //std::cout<<D<<std::endl ;
    for ( int i =0  ; i < 10 ;++i)
        BOOST_CHECK(D[i] == i );

    cntree::vector_tree_pool_cnc <int32_t,std::allocator<uint32_t> > K (D);
    BOOST_CHECK ( K.size() == 10);
    K.clear() ;
    K = D ;
    BOOST_CHECK ( K.size() == 10);

    cntree::vector_tree_pool_cnc <int32_t,std::allocator<uint32_t> > Q ( std::move ( D));
    BOOST_CHECK ( Q.size() == 10 and D.size() == 10 );
    D = std::move( Q);
    BOOST_CHECK ( Q.size() == 10 and D.size() == 10 );
    ...
    //-------------------------------------------------------------------
    cntree::vector_tree_pool<int32_t> H( std::move(A));
    BOOST_CHECK (H.check() );
    BOOST_CHECK (A.check() );
    ...
    cntree::vector_tree_pool<int32_t>::iterator PAux  ;
    for ( int32_t i = 0 ; i < 10 ; i ++ )
    ...
    cntree::vector_tree_pool<int32_t>::reverse_iterator Alfa , Beta ;
...
void prueba2 ( void)
{   //---------------------- Variables----------------------------
    cntree::vector_tree_pool<int> M1 , M2;
...
{   //---------------------- Variables----------------------------
    ...
    cntree::vector_tree_pool<int> M1 ;
    for ( i = 0 ; i < 1000; i ++ )
...
void prueba4 ( void)
{   //---------------------- Variables----------------------------
    cntree::vector_tree_pool<int> M1, M2, M3 ;
       int i ;
...
void prueba5 ( void)
{   //---------------------------- begin -------------------------
    cntree::vector_tree_pool<int >  V1, V2, V3 ;
    std::vector<int> V4 ;
...
void prueba7 ( void)
{   //--------------------------- begin --------------------------------------
    cntree::vector_tree_pool<int> V1, V2,V3,V4 ;
