...
cntree::set_cnc <uint32_t> VTC ;
typedef cntree::set_cnc <uint32_t>::iterator s_iterator ;
...
void prueba1()
{   //------------------ begin ---------------
    cntree::set_cnc<int> s;
    int i ;
    cntree::set_cnc<int>::iterator Alfa, Beta;
    s.insert(1);
    s.insert(3);
    s.insert(5);
    s.insert(7);
    ...
    // Pruebas con rvalues
    //---------------------------------------------------------
    cntree::set_cnc<int> s2( std::move (s));

    for ( i = 1 , Alfa = s2.begin();Alfa != s2.end(); ++Alfa, i+=2)
        BOOST_CHECK ( *Alfa == i );
    s = std::move(s2);
    s2= s ;
    cntree::set_cnc<int>::reverse_iterator Delta ;
    ...
    // Pruebas con rvalues
    //-----------------------------------------------------
    cntree::multiset_cnc<int> ms2(std::move (ms));
...
{   //------------------- begin -------------------------
    ...
    cntree::set_cnc<int> A ;
    for ( int i = 0 ; i< NELEM ; ++i)
        A.insert( V[i] );

    cntree::set_cnc<int> B(A) ;
    BOOST_CHECK ( A.size() == B.size());
    cntree::set_cnc<int> C ( B.begin() + 20 , B.end() -20);
    B = C ;
    BOOST_CHECK ( C.size() == B.size() and A.size() != B.size());

    cntree::set_cnc<int>::iterator Alfa = A.begin() ;
    ...
    cntree::set_cnc<int>::iterator Beta ;
    ...
    std::pair<cntree::set_cnc<int>::iterator,cntree::set_cnc<int>::iterator> Gama ;
...
{   //-------------------- begin -------------------------
    ...
    cntree::multiset_cnc<int> B(A) ;
    BOOST_CHECK  (A.size() == B.size() );
    cntree::multiset_cnc<int> C ( B.begin() + 20 , B.end() -20);
    B = C ;
    BOOST_CHECK  (C.size() == B.size() and A.size() != B.size() );

    cntree::multiset_cnc<int>::iterator Alfa = A.begin() ;
    for (int  i = 0 ; Alfa != A.end() ; ++Alfa, ++i )
    {   BOOST_CHECK ( (*Alfa) == A.pos(i) );;
    };
    B.clear();
    ...
    cntree::multiset_cnc<int>::iterator Beta ;
    Alfa = A.lower_bound( *(A.begin() +10)-1 );
    Beta = A.upper_bound ( A.pos ( A.size() -10) +1);
    signed_type Dif = Beta - Alfa ;
    for ( int32_t i =0 ; i < Dif ; Alfa++,i++);
    ...
    std::pair<cntree::multiset_cnc<int>::iterator,
              cntree::multiset_cnc<int>::iterator> Gama ;
    Gama = A.equal_range(*(A.begin() +10) -1);
    Dif = Beta - Alfa ;
    for ( int32_t i =0 ; i < Dif ; Alfa++,i++);
    BOOST_CHECK ( Alfa == Beta );
...
{   //------------------ Variables----------------
    ...
    cntree::set_cnc<int>::iterator Gamma ;
    for ( i = 0 ; i < M1.size() ; ++i)
    ...
    std::pair < cntree::set_cnc<int>::iterator, cntree::set_cnc<int>::iterator > PI ;
...
{   //---------------------- Variables----------------------------
    ...
    cntree::multiset_cnc<int>::iterator Gamma ;
    for ( i = 0 ; i < 1000 ; ++i)
    {   Gamma = M1.find( 1000+ i * 2);
    ...
    std::pair < cntree::multiset_cnc<int>::iterator, cntree::multiset_cnc<int>::iterator > PI ;
