...
=======
cntree_map ( InputIterator first     ,    InputIterator last      ,
      const comp_key_t& C1 = comp_key_t() ,
      const alloc_t & A1= alloc_t()  )  : st ( C1, A1 )
>>>>>>> a638c30ad722b2664968a5babd793174f3466a44
{   //---------------------begin ----------------------------
...
=======
//  mypair insert ( const value_type& val )
//
//  mypair insert_if ( std::function < bool ( const value_type & )>  M,
...
//
//  template <class P >
//  mypair insert_if ( std::function < bool ( const value_type & )>  M ,P && val)
>>>>>>> a638c30ad722b2664968a5babd793174f3466a44
//
//  template <class P >
...
{   //---------------------------- begin ---------------------------------
    barrier_modify BM ( BD);
    connector C ;
    node_t *PAux = st.connector_norep( st.fltr(val) , C);
    bool SW = ( PAux == NULL);
    if ( SW )
    {   PAux = st.allocate (1) ;
        st.construct ( PAux , val  );
>>>>>>> a638c30ad722b2664968a5babd793174f3466a44
        BM.wait_no_readers() ;
        st.connect( PAux, C);
    };
...
{   //---------------------------- begin ---------------------------------
    ...
    if ( SW )
    {   PAux = st.allocate (1) ;
        st.construct ( PAux , std::move (val)  );
        if ( M ( PAux->data))
        {   BM.wait_no_readers() ;
            st.connect( PAux, C);
            I =st.get_it ( PAux);
...
/// @return none
//---------------------------------------------------------------------------
mypair erase_if ( const_iterator iter , std::function <bool(const  value_type &)>( M1))
>>>>>>> a638c30ad722b2664968a5babd793174f3466a44
{   //---------- begin -------------
    barrier_modify BM ( BD);
...
//  bool modify ( const key_type& x ,const data_t & val)
//  bool modify ( const key_type& x ,std::function<void( value_type &)>  M)
//  void modify_pos ( signed_type Pos ,const data_t & val)
...
/// @param [in] Prim : position to the first element to erase
/// @param [in] Position to the last element to erase
/// @return true : modify false : don't exist
/// @remarks
//---------------------------------------------------------------------------
bool modify ( const key_type& x ,std::function<void( value_type &)>  M)
>>>>>>> a638c30ad722b2664968a5babd793174f3466a44
...
mypair insert_or_modify ( const key_type& x ,const data_t &D ,Function &&  M)
=======
/// @param [in] M : function to modify the existing element
/// @return  pair <iterator,bool> with an iterator to the element inserted or
///          modified, and the value true means inserted and the false modified
//---------------------------------------------------------------------------
mypair insert_or_modify ( const key_type& x ,const data_t &D ,
